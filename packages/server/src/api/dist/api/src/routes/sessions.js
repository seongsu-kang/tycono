...
import { createSession, getSession, listSessions, deleteSession, deleteMany, deleteEmpty, updateSession, addMessage, } from '../services/session-store.js';
...
sessionsRouter.get('/:id/stream', (req, res) => {
    ...
    const fromSeq = parseInt(req.query.from ?? '0', 10);
    res.writeHead(200, {
    ...
    if (!exec || !isMessageActive(exec.status)) {
