...
import { isMessageActive } from '../../../shared/types.js';
    ...
    try {
        ...
        const waves = files
            ...
            try {
                ...
                const hasRunning = roles.some((r) => r.status && isMessageActive(r.status));
                return {
                    id,
                    timestamp: id,
 ...
 */
operationsRouter.get('/traces/:jobId', (req, res, next) => {
    try {
        ...
        const traceId = events[0]?.traceId ?? events.find(e => e.data?.traceId)?.data?.traceId ?? jobId;
        if (!includeChain) {
 ...
 */
operationsRouter.get('/traces', (req, res, next) => {
    try {
        ...
        for (const jid of allJobIds) {
            ...
            if (!startEvent)
                continue;
            const traceId = events[0]?.traceId ?? startEvent?.data?.traceId ?? jid;
            if (roleFilter && startEvent.roleId !== roleFilter)
                continue;
            if (!traces.has(traceId)) {
                ...
                traces.set(traceId, {
                    ...
                    rootTask: (startEvent.data.task ?? '').slice(0, 200),
                    jobCount: 1,
                    status,
                });
            }
