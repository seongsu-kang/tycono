...
import { getSession, createSession, addMessage, updateMessage, listSessions, } from '../services/session-store.js';
import { executionManager } from '../services/execution-manager.js';
import { messageStatusToRoleStatus, eventTypeToMessageStatus } from '../../../shared/types.js';
...
export function handleExecRequest(req, res) {
    ...
    if (method === 'GET' && waveDetailMatch) {
        ...
        const active = activeWaves.find((w) => w.waveId === waveId);
        if (active) {
...
/* ─── POST /api/jobs ─────────────────────── */
async function handleStartJob(body, res) {
    ...
    if (type === 'wave') {
        ...
        {
            const state = supervisorHeartbeat.start(`wave-${Date.now()}`, actualDirective, targetRoles && targetRoles.length > 0 ? targetRoles : undefined, continuous, preset, modelOverrides);
    ...
    if (!readOnly && waveId) {
        try {
            ...
            if (decision.action === 'amend' && decision.prevSessionId) {
                ...
                const amendedExec = executionManager.continueSession(decision.prevSessionId, `[FOLLOW-UP from ${sourceRole}] ${task}`, sourceRole);
                if (amendedExec) {
                    jsonResponse(res, 200, {
    ...
    const sessionSource = waveId ? 'wave' : 'dispatch';
...
function handleSaveWave(body, res) {
    const directive = body.directive;
    let sessionIds = (body.sessionIds ?? body.jobIds);
    const waveId = body.waveId;
...
function handleWavePreview(body, res) {
    const directive = body.directive ?? '';
    const targetRoles = body.targetRoles;
    const continuous = body.continuous === true;
    ...
    const activeCLevels = targetRoles
        ...
        : cLevelIds;
    function buildRolePreview(roleId) {
    ...
    const team = activeCLevels.map(id => buildRolePreview(id)).filter((r) => r !== null);
    // Count total agents
    ...
    function estimateRoleCost(role) {
...
function handleWave(body, req, res) {
    ...
    if (activeWaves.length > 0) {
        ...
        const ceoSession = listSessions().find(s => s.waveId === existingWave.id && s.roleId === 'ceo' && s.status === 'active');
        if (ceoSession) {
            const amendedExec = executionManager.continueSession(ceoSession.id, `[ADDITIONAL DIRECTIVE] ${directive}`, 'user');
            if (amendedExec) {
                jsonResponse(res, 200, {
 ...
 */
function handleWaveSupervisor(directive, targetRoles, continuous, req, res, preset, modelOverrides) {
    const state = supervisorHeartbeat.start(`wave-${Date.now()}`, directive, targetRoles && targetRoles.length > 0 ? targetRoles : undefined, continuous, preset, modelOverrides);
    if (state.status === 'error') {
        jsonResponse(res, 500, { error: 'Failed to start supervisor' });
        return;
    }
...
function handleWaveDirective(waveId, body, res) {
    const text = body.text ?? body.directive;
    if (!text) {
...
function handleWaveQuestion(waveId, body, res) {
    ...
    if (activeExecs.length === 0) {
        ...
        for (const ses of recentActive) {
            ...
            if (isDone)
                continue;
            const startEvent = events.find(e => e.type === 'msg:start');
            const task = startEvent?.data?.task ?? ses.title ?? '';
            recovered.push({
                id: `recovered-${ses.id}`,
...
function handleSessionMessage(sessionId, body, req, res) {
    ...
    const unwatchExecs = executionManager.onExecutionCreated((childExec) => {
        if (childExec.type !== 'assign')
            return;
        if (roleMsg.status !== 'streaming')
            return;
        if (!pendingDispatches.has(childExec.roleId))
            return;
    ...
    for (const e of executionManager.listExecutions({ active: true })) {
        const mapped = messageStatusToRoleStatus(e.status);
        if (teamStatus[e.roleId]?.status === 'working' && mapped === 'awaiting_input')
            continue;
        teamStatus[e.roleId] = { status: mapped, task: e.task };
    }
    const handle = getRunner().execute({ companyRoot: COMPANY_ROOT, roleId, task: fullTask, sourceRole: 'ceo', orgTree, readOnly, model: orgTree.nodes.get(roleId)?.model, attachments, teamStatus, sessionId }, {
        onText: (text) => {
