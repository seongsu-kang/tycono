...
syncRouter.get('/roles', (_req, res, next) => {
    try {
        const tree = buildOrgTree(COMPANY_ROOT);
        const trackedRoles = [];
        for (const [id, node] of tree.nodes) {
            if (id === 'ceo' || !node.source)
                continue;
            trackedRoles.push({
                roleId: id,
                name: node.name,
...
syncRouter.post('/apply', async (req, res, next) => {
    try {
        const { roleId, changes, upstreamVersion } = req.body;
        if (!roleId || !changes) {
        ...
        if (upstreamVersion) {
            ...
            if (raw.source && typeof raw.source === 'object') {
                raw.source.upstream_version = upstreamVersion;
            }
...
syncRouter.get('/stats', (_req, res, next) => {
    try {
        const tree = buildOrgTree(COMPANY_ROOT);
