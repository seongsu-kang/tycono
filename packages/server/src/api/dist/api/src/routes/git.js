 ...
 */
function resolveRepoRoot(repo = 'akb') {
    if (repo === 'akb') {
        return COMPANY_ROOT;
    }
...
gitRouter.get('/status', (req, res, next) => {
    try {
        const repoParam = req.query.repo ?? 'akb';
        const repo = repoParam === 'code' ? 'code' : 'akb';
        let repoRoot;
        try {
...
gitRouter.delete('/worktree/{*path}', (req, res, next) => {
    try {
...
gitRouter.delete('/branch/{*name}', (req, res, next) => {
    try {
