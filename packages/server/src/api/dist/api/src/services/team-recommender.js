 ...
 */
import { buildOrgTree, getSubordinates, getDescendants } from '../engine/org-tree.js';
 ...
 */
function mapDomainsToRoles(domains, orgTree) {
...
async function classifyDirective(text) {
    try {
        ...
        if (engine === 'claude-cli') {
            ...
            const response = await provider.chat(CLASSIFY_SYSTEM, [{ role: 'user', content: text }]);
...
function classifyDirectiveFallback(text) {
    const t = text.toLowerCase();
    const domains = [];
    // Keyword-based domain detection
    if (/코드|구현|개발|빌드|build|implement|develop|fix|bug|api|서버|deploy|refactor/.test(t))
        domains.push('code');
    if (/디자인|design|ui|ux|css|스타일|비주얼|visual|레이아웃/.test(t))
        domains.push('design');
    if (/기획|plan|prd|스펙|spec|기능\s*정의|요구사항|밸런/.test(t))
        domains.push('planning');
    if (/테스트|test|qa|검증|verify|플레이테스트/.test(t))
        domains.push('testing');
    if (/사업|business|매출|revenue|시장|market|경쟁|전략|strategy|pricing/.test(t))
        domains.push('business');
    if (/조사|research|분석|analy|리서치/.test(t))
        domains.push('research');
    if (/문서|doc|readme|작성|write|블로그|blog/.test(t))
...
function writeTeamPresets(data) {
    const filePath = TEAM_PRESETS_PATH();
...
export function saveCustomTeam(name, roles) {
...
export function incrementTeamUsage(teamId) {
    const data = readTeamPresets();
...
/* ─── Main: Recommend ────────────────────────── */
export async function recommendTeam(directive) {
    // 1. Build org tree
