 ...
 */
function buildDefaultPreset(companyRoot) {
 ...
 */
export function loadPresets(companyRoot) {
    const presets = [];
    ...
    if (fs.existsSync(defaultYamlPath)) {
        try {
            const raw = YAML.parse(fs.readFileSync(defaultYamlPath, 'utf-8'));
            if (raw.name)
                defaultPreset.definition.name = raw.name;
            if (raw.tagline)
                defaultPreset.definition.tagline = raw.tagline;
            if (raw.description)
                defaultPreset.definition.description = raw.description;
        }
    ...
    for (const agenciesDir of agencyDirs) {
        ...
        for (const entry of entries) {
            if (!entry.isDirectory())
                continue;
            if (loadedIds.has(entry.name))
                continue; // earlier sources take priority
    ...
    if (fs.existsSync(bundledPresetsDir)) {
        ...
        for (const entry of entries) {
            if (!entry.isDirectory())
                continue;
            if (loadedIds.has(entry.name))
                continue; // user's preset takes priority
 ...
 */
export function getPresetSummaries(companyRoot) {
 ...
 */
export function getPresetById(companyRoot, presetId) {
 ...
 */
function downloadPreset(companyRoot, presetId) {
    ...
    try {
        ...
        const response = execSync(`curl -s --max-time 10 "${REGISTRY_URL}/${presetId}/download"`, { encoding: 'utf-8' });
        ...
        // Write preset.yaml
        fs.writeFileSync(path.join(targetDir, 'preset.yaml'), YAML.stringify(data.preset));
        ...
        console.warn(`[Preset] Failed to download "${presetId}": ${err.message}`);
        return null;
    }
}
/**
 ...
 */
export function autoSelectPreset(companyRoot, directive) {
    ...
    for (const preset of presets) {
        ...
        for (const uc of useCases) {
            ...
            for (const ucw of ucWords) {
                if (words.includes(ucw))
                    score += 2;
            }
        }
        // category + industry: exact match only (weight 1)
        if (def.category && words.includes(def.category.toLowerCase()))
            score += 1;
        if (def.industry && words.includes(def.industry.toLowerCase()))
            score += 1;
        if (score > bestScore) {
            bestScore = score;
            bestId = def.id;
