...
/* ─── Default Appearances ─── */
const DEFAULT_ROLE_APPEARANCES = {
...
export function loadTeam(teamName) {
 ...
 */
function installSkill(root, skillId) {
 ...
 */
function installTeamSkills(root, roles) {
    const installed = [];
...
export function scaffold(config) {
    ...
    if (config.apiKey) {
        ...
        const companyConfig = { engine: 'claude-cli', codeRoot: defaultCodeRoot };
        writeConfig(root, companyConfig);
        created.push('.tycono/config.json');
    }
    ...
    if (config.existingProjectPath) {
        ...
        if (!fs.existsSync(targetDir)) {
            ...
            fs.writeFileSync(path.join(targetDir, 'prd.md'), `# Existing Project\n\nImported from: ${config.existingProjectPath}\n`);
...
function createRole(root, role) {
    ...
    yamlLines.push('authority:', '  autonomous:', '    - Implementation within assigned scope', '  needs_approval:', '    - Architecture changes', 'knowledge:', '  reads:', '    - projects/', '  writes:', '    - projects/', 'reports:', '  daily: standup', '  weekly: summary');
