...
import { isMessageTerminal } from '../../../shared/types.js';
/* ─── Session directory ─────────────────── */
...
export function getSession(id) {
    ...
    if (cached.messages.length === 0 && cached.status !== 'active') {
        ...
        if (full)
            return full; // Return disk version (don't cache — it's a one-time read)
    }
    return cached;
}
...
export function deleteSession(id, force = false) {
    const session = cache.get(id);
    if (!session)
        return false;
