 ...
 */
export function findActiveSession(waveId, roleId) {
    const session = listSessions().find(s => s.waveId === waveId && s.roleId === roleId && (s.status === 'active' || s.status === 'awaiting_input'));
    if (!session)
        return null;
 ...
 */
export function findPrevDoneSession(waveId, roleId) {
    const sessions = listSessions().filter(s => s.waveId === waveId && s.roleId === roleId && (s.status === 'done' || s.status === 'closed'));
 ...
 */
function countErrorSessions(waveId, roleId) {
    return listSessions().filter(s => s.waveId === waveId && s.roleId === roleId && s.status === 'error').length;
}
/* ─── Decision Logger ─────────────────── */
 ...
 * 4. 세션 없음 → new (첫 생성)
 */
export async function decideDispatchOrAmend(waveId, roleId, sourceRole, newTask) {
