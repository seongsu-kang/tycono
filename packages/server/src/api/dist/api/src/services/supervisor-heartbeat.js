...
import { createSession, getSession, listSessions, addMessage } from './session-store.js';
...
class SupervisorHeartbeat {
     ...
     */
    start(waveId, directive, targetRoles, continuous = false, preset, modelOverrides) {
     ...
     */
    saveWaveFile(waveId, directive, preset, modelOverrides) {
        try {
            ...
            if (!fs.existsSync(wavePath)) {
                ...
                if (preset)
                    waveData.preset = preset;
                if (modelOverrides && Object.keys(modelOverrides).length > 0)
                    waveData.modelOverrides = modelOverrides;
     ...
     */
    answerQuestion(waveId, questionId, answer) {
        const state = this.supervisors.get(waveId);
        if (!state)
            return false;
     ...
     */
    markDirectivesDelivered(waveId) {
        const state = this.supervisors.get(waveId);
        if (!state)
            return;
     ...
     */
    addQuestion(waveId, question, fromRole, context) {
        const state = this.supervisors.get(waveId);
        const q = {
            id: `q-${Date.now()}`,
            question,
     ...
     */
    static CLASSIFY_SYSTEM = `You classify user messages to a CEO AI assistant.
...
"deploy the app" → T`;
    async classifyDirective(text) {
        ...
        try {
            ...
            if (engine === 'claude-cli') {
                ...
                const response = await provider.chat(SupervisorHeartbeat.CLASSIFY_SYSTEM, [{ role: 'user', content: text }]);
    ...
    isConversationDirectiveFallback(text) {
        const t = text.trim();
        ...
        if (taskPatterns.some(p => p.test(t)))
            return false;
    ...
    loadWaveHistory(waveId) {
        try {
            ...
            const formatted = recent.map(e => e.role === 'ceo' ? `CEO: "${e.text}"` : `→ ${e.text}`).join('\n');
    ...
    spawnConversation(state, directive) {
