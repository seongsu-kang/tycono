...
function extractTldr(content) {
    ...
    if (blockquote)
        return blockquote[1].trim().slice(0, 120);
    const tldrSection = content.match(/##\s+TL;DR\s*\n+([^\n#]+)/i);
    if (tldrSection)
        return tldrSection[1].trim().slice(0, 120);
    const firstLine = content.split('\n').find((l) => l.trim().length > 0 && !l.startsWith('#') && !l.startsWith('---') && !l.startsWith('|'));
    return firstLine?.trim().slice(0, 120) ?? '';
}
function extractFrontmatterCategory(filePath) {
    let raw;
    try {
    ...
    const summary = data.summary || data.tldr || extractTldr(content);
    return {
        category,
        title,
        summary,
...
async function processDocumentWithCli(filePath) {
    let content;
    try {
    ...
    try {
        ...
        const akbType = parsed.akb_type === 'hub' ? 'hub' : 'node';
...
async function processDocumentWithLLM(filePath, llm) {
    let content;
    try {
        content = fs.readFileSync(filePath, 'utf-8');
    }
    ...
    const userMessage = `File: ${fileName}\n\n${truncated}`;
    try {
        const response = await llm.chat(CLASSIFY_PROMPT, [{ role: 'user', content: userMessage }], undefined);
...
function processDocumentSimple(filePath) {
    let content;
    try {
        content = fs.readFileSync(filePath, 'utf-8');
    }
...
function buildAkbContent(result, sourceFile) {
...
export async function importKnowledge(paths, companyRoot, callbacks, llm) {
    ...
    for (let i = 0; i < allFiles.length; i++) {
        ...
        let result = extractFrontmatterCategory(file);
        if (!result && llm) {
...
/* ─── Hub Updater ────────────────────────────── */
function updateKnowledgeHub(companyRoot, entries, existingContent) {
    if (entries.length === 0)
        return;
