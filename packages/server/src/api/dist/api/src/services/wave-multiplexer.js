...
class WaveMultiplexer {
    clients = new Map();
    waveSessions = new Map();
    registerSession(waveId, execution) {
    ...
    subscribeSessionToClient(waveId, client, execution, sendNotification) {
        ...
        if (existing) {
            if (existing.executionId === execution.id)
                return; // Same execution, skip
        ...
        const subscriber = (event) => {
            ...
            if (client.sentEvents.size > 5000) {
                ...
                for (const e of entries.slice(-2000))
                    client.sentEvents.add(e);
            }
            const waveSeq = client.waveSeq++;
            sendSSE(client, 'wave:event', {
                waveId,
    ...
    onExecutionCreated(execution) {
        ...
        if (!waveId && execution.parentSessionId) {
            ...
            if (parentSession?.waveId)
                waveId = parentSession.waveId;
        }
        if (!waveId)
            return;
    ...
    findWaveIdForSession(sessionId) {
    ...
    getActiveWaves() {
        ...
        for (const [waveId, sessions] of this.waveSessions) {
            ...
            const startedAt = Math.min(...Array.from(sessions.values()).map(e => new Date(e.createdAt).getTime()));
