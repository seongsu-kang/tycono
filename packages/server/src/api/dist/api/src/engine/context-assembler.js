...
import { isRoleActive } from '../../../shared/types.js';
export function assembleContext(companyRoot, roleId, task, sourceRole, orgTree, options) {
    ...
    const pushSection = (text, cacheable) => taggedSections.push({ text, cacheable });
...
- Example: \`git worktree add .worktrees/feature-xyz -b feature/xyz\` (from cwd, which is already code repo)`, true);
    ...
    if (options?.priorDispatches && options.priorDispatches.length > 0) {
        ...
        if (sameRole.length > 0) {
            const summaries = sameRole.map((d, i) => `### Session ${i + 1}\n**Task**: ${d.task.slice(0, 200)}\n**Result**: ${d.result.slice(0, 500)}`).join('\n\n');
 ...
 */
function loadPresetKnowledge(companyRoot, presetId) {
    const candidateDirs = [
        ...
        try {
            ...
            for (const file of entries) {
                if (seenFiles.has(file))
                    continue; // deduplicate: first source wins
                seenFiles.add(file);
                if (seenFiles.size > 10)
                    break; // Cap at 10 docs total
...
function buildPersonaSection(node) {
    return `# Persona
...
function buildAuthoritySection(node) {
...
function buildKnowledgeSection(node) {
    const reads = node.knowledge.reads.map((p) => `- \`${p}\``).join('\n');
    const writes = node.knowledge.writes.map((p) => `- \`${p}\``).join('\n');
    const hasKnowledgeWrite = node.knowledge.writes.some((p) => p === '*' || p.startsWith('knowledge') || p === 'knowledge/*');
...
function loadHubSummaries(companyRoot, node) {
...
function loadCeoDecisions(companyRoot) {
...
function buildSupervisionSection(node) {
...
function buildKnowledgeManagementSection(roleId) {
