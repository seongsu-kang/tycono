...
export class ClaudeCliRunner {
    execute(config, callbacks) {
        ...
        if (config.sessionId) {
            ...
            if (parentSes?.waveId)
                cleanEnv.DISPATCH_WAVE_ID = parentSes.waveId;
        }
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            proc.on('exit', (code, signal) => {
                ...
                setTimeout(() => {
                    if (!resolved) {
                        ...
                        try {
                            fs.unlinkSync(dispatchScript);
                        }
                        catch { /* ignore */ }
                        try {
                            fs.unlinkSync(consultScript);
                        }
                        catch { /* ignore */ }
                        try {
                            fs.rmSync(runnerOutputDir, { recursive: true, force: true });
                        }
                        catch { /* ignore */ }
                        resolve({
                            output,
            ...
            proc.on('close', (code, signal) => {
                ...
                try {
                    fs.unlinkSync(supervisionScript);
                }
                catch { /* ignore */ }
                try {
                    fs.rmSync(runnerOutputDir, { recursive: true, force: true });
...
function processStreamEvent(event, callbacks, handlers) {
            ...
            if (Array.isArray(content)) {
                for (const block of content) {
                    if (block.type === 'text' && typeof block.text === 'string') {
                        ...
                        handlers.addToolCall(block.name, block.input);
