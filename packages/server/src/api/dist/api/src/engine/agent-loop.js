        ...
        if (typeof msg.content === 'string') {
            ...
            for (let i = 0; i < msg.content.length; i++) {
                ...
                if (block.type === 'tool_result') {
                    ...
                    block.text = block.text.slice(0, 300) + '\n[... compressed ...]';
                }
            }
        }
    }
...
export async function runAgentLoop(config) {
    const { companyRoot, roleId, task, sourceRole, orgTree, readOnly = false, maxTurns = 20, abortSignal, onText, onToolExec, onDispatch: onDispatchCallback, onConsult: onConsultCallback, onTurnComplete, } = config;
    ...
    const toolExecOptions = {
        ...
        onConsult: async (targetRoleId, question) => {
            // Authority check
    ...
    while (turns < maxTurns) {
        ...
        const parallelResults = await Promise.all(parallelCalls.map(tc => executeTool({ id: tc.id, name: tc.name, input: tc.input }, toolExecOptions)));
    ...
    if (!readOnly && depth === 0 && turns > 0) {
        ...
        if (isCLevel && dispatches.length > 0) {
            const dispatchSummary = dispatches.map((d, i) => `${i + 1}. **${d.roleId}**: "${d.task.slice(0, 80)}"\n   Result: ${d.result.slice(0, 300)}`).join('\n\n');
            ...
            for (let round = 0; round < maxSupervisionRounds && turns < maxTurns; round++) {
                ...
                if (supResponse.stopReason !== 'tool_use')
                    break;
                // Execute tool calls
                const supToolCalls = supResponse.content.filter((b) => b.type === 'tool_use');
                const supResults = [];
        ...
        if (hasFileChanges || dispatches.length > 0) {
            ...
            for (let round = 0; round < maxPostKRounds && turns < maxTurns; round++) {
                ...
                if (postKResponse.stopReason !== 'tool_use')
                    break;
                // Execute Post-K tool calls
                const postKToolCalls = postKResponse.content.filter((b) => b.type === 'tool_use');
                const postKResults = [];
