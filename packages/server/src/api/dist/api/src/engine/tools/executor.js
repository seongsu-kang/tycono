 ...
 */
function isAllowedReadPath(absolute, companyRoot) {
    const allowedPaths = [
        companyRoot,
...
function readFile(id, input, companyRoot, roleId, orgTree) {
...
function listFiles(id, input, companyRoot, roleId, orgTree) {
...
function searchFiles(id, input, companyRoot, roleId, orgTree) {
...
function writeFile(id, input, companyRoot, roleId, orgTree) {
    const filePath = String(input.path ?? '');
...
function editFile(id, input, companyRoot, roleId, orgTree) {
    const filePath = String(input.path ?? '');
...
/* ─── Dispatch / Consult ─────────────────────── */
async function dispatchTask(id, input, onDispatch, options) {
...
function findWaveIdForSessions(sessionIds) {
    for (const sid of sessionIds) {
        const session = getSession(sid);
        if (session?.waveId)
            return session.waveId;
    }
    return undefined;
}
/**
 ...
 */
async function heartbeatWatch(id, input, companyRoot) {
    ...
    const durationSec = Math.min(Math.max(Number(input.durationSec) || DEFAULT_WATCH_DURATION, 5), MAX_WATCH_DURATION);
    const alertOn = input.alertOn ?? ['msg:done', 'msg:error', 'msg:awaiting_input'];
    const alertSet = new Set(alertOn);
    // Collect current checkpoints (last known seq for each session)
    const startCheckpoints = new Map();
    for (const sid of sessionIds) {
 ...
 */
function amendSession(id, input, onAmend) {
 ...
 */
function abortSession(id, input, onAbort) {
    const sessionId = String(input.sessionId ?? '');
