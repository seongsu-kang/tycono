...
import { buildOrgTree, assembleContext, validateDispatch, RoleLifecycleManager, formatOrgChart, } from '../engine/index.js';
import { createRunner } from '../engine/runners/index.js';
export const engineRouter = Router();
/* ─── GET /api/engine/org — Org tree ─────────── */
engineRouter.get('/org', (_req, res, next) => {
    try {
...
engineRouter.get('/context/:roleId', (req, res, next) => {
    try {
        const roleId = String(req.params.roleId ?? '');
        const sourceRole = String(req.query.source ?? 'ceo');
        const task = String(req.query.task ?? '(preview — no task specified)');
        const tree = buildOrgTree(COMPANY_ROOT);
        const context = assembleContext(COMPANY_ROOT, roleId, task, sourceRole, tree);
        res.json({
...
engineRouter.post('/dispatch/validate', (req, res, next) => {
    try {
...
engineRouter.post('/roles', async (req, res, next) => {
    try {
        const def = req.body;
...
engineRouter.patch('/roles/:id', async (req, res, next) => {
    try {
...
engineRouter.delete('/roles/:id', async (req, res, next) => {
    try {
        const id = String(req.params.id);
        const manager = new RoleLifecycleManager(COMPANY_ROOT);
...
engineRouter.get('/roles/validate', (_req, res, next) => {
    try {
        const manager = new RoleLifecycleManager(COMPANY_ROOT);
...
engineRouter.post('/roles/:id/skill/regenerate', async (req, res, next) => {
    try {
        const id = String(req.params.id);
        const manager = new RoleLifecycleManager(COMPANY_ROOT);
...
engineRouter.post('/ask/:roleId', async (req, res, next) => {
    try {
        const roleId = String(req.params.roleId);
