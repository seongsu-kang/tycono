import { canDispatchTo, canConsult, getChainOfCommand } from './org-tree.js';
/* ─── Validation ─────────────────────────────── */
/**
 * Validate whether a source role can dispatch a task to a target role.
 * Returns a detailed result with the reason.
 */
export function validateDispatch(orgTree, sourceRole, targetRole) {
 ...
 */
export function validateConsult(orgTree, sourceRole, targetRole) {
    if (sourceRole === targetRole) {
 ...
 */
export function validateWrite(orgTree, roleId, filePath) {
 ...
 */
export function validateRead(orgTree, roleId, filePath) {
    const node = orgTree.nodes.get(roleId);
    if (!node) {
        return { allowed: false, reason: `Role not found: ${roleId}` };
    }
