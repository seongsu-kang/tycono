...
import { getSession, createSession, addMessage, updateMessage as updateSessionMessage, appendMessageEvent } from './session-store.js';
 ...
 */
function resolveTargetRole(sourceRole, parentJobId, jobs) {
    ...
    if (parentJobId) {
        ...
        if (parentJob && parentJob.roleId !== 'ceo')
            return parentJob.roleId;
    }
    // Default: CEO
    return 'ceo';
}
...
class JobManager {
    ...
    async initializeAndRunJob(job, params, orgTree) {
        try {
        ...
        const handle = this.runner.execute({
            ...
            onTurnComplete: (turn) => {
                ...
                if (!softLimitWarned && harnessTurnCount >= limits.softLimit) {
                    ...
                    console.warn(`[Harness] Job ${job.id} (${params.roleId}): turn ${harnessTurnCount} reached softLimit (${limits.softLimit})`);
                ...
                if (harnessTurnCount >= limits.hardLimit) {
                    ...
                    console.warn(`[Harness] Job ${job.id} (${params.roleId}): turn ${harnessTurnCount} reached hardLimit (${limits.hardLimit}). Pausing for approval.`);
                ...
                if (changedMdFiles.length > 0) {
                    try {
                        ...
                        if (!pkResult.pass) {
                            ...
                            console.log(`[Post-K] Job ${job.id} (${params.roleId}): ${pkResult.debt.length} knowledge debt item(s)`);
    ...
    updateSessionRoleMessage(job, text) {
        if (!job.sessionId)
            return;
    ...
    embedSessionEvent(job, type, data) {
        if (!job.sessionId)
            return;
        const session = getSession(job.sessionId);
        if (!session)
    ...
    finalizeSessionMessage(job, status, result) {
        if (!job.sessionId)
            return;
        const session = getSession(job.sessionId);
        if (!session)
    ...
    cleanupOrphanedChildren(parentJobId) {
    ...
    replyToJob(id, response, responderRole) {
        const job = this.jobs.get(id);
        if (!job || (job.status !== 'awaiting_input' && job.status !== 'done'))
            return null;
    ...
    recoverJobFromStreams(sessionId) {
        try {
