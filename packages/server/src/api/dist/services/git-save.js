 ...
 */
function resolveRepoRoot(akbRoot, repo = 'akb') {
    if (repo === 'akb') {
        return akbRoot;
    }
 ...
 */
function shouldFilterPaths(repo = 'akb') {
    return repo === 'akb';
}
 ...
 */
export function gitInit(root, repo = 'akb') {
    if (!isGitAvailable()) {
 ...
 * @param repo - Repository type ('akb' or 'code'), default 'akb'
 */
export function getGitStatus(root, repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot)) {
        return {
            dirty: false,
    ...
    if (logLine) {
        ...
        if (sha)
            lastCommit = { sha, message: message ?? '', date: date ?? '' };
    }
 ...
 * @param repo - Repository type ('akb' or 'code'), default 'akb'
 */
export function gitSave(root, message, repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot)) {
 ...
 * @param repo - Repository type ('akb' or 'code'), default 'akb'
 */
export function gitHistory(root, limit = 20, repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot))
        return [];
 ...
 * @param repo - Repository type ('akb' or 'code'), default 'akb'
 */
export function gitRestore(root, sha, paths, repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot)) {
 ...
 * @param repo - Repository type ('akb' or 'code'), default 'akb'
 */
export function gitFetchStatus(root, repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot)) {
 ...
 * @param repo - Repository type ('akb' or 'code'), default 'akb'
 */
export function gitPull(root, repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot)) {
 ...
 */
export function githubStatus(root, repo = 'akb') {
    // Check gh CLI
    ...
    try {
        ...
        const output = err instanceof Error ? err.stderr ?? '' : '';
 ...
 */
export function githubCreateRepo(root, repoName, visibility = 'private', repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot)) {
    ...
    try {
        ...
        const result = execSync(`gh repo create "${repoName}" ${flag} --source=. --remote=origin --push`, { cwd: repoRoot, encoding: 'utf-8', timeout: 30000, stdio: ['pipe', 'pipe', 'pipe'] }).trim();
 ...
 */
export function gitAddRemote(root, url, repo = 'akb') {
    const repoRoot = resolveRepoRoot(root, repo);
    if (!isGitRepo(repoRoot)) {
