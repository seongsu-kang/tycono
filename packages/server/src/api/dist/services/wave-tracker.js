...
export function appendFollowUpToWave(waveId, jobId, roleId, task, sessionId) {
...
export function updateFollowUpForReply(waveId, roleId, oldJobId, newJobId, sessionId) {
    const waveFile = findWaveFile(waveId);
    if (!waveFile)
        return;
    try {
        ...
        let idx = -1;
        if (oldJobId) {
            idx = data.roles.findIndex((r) => r.jobId === oldJobId);
        }
        if (idx < 0) {
...
export function updateFollowUpInWave(waveId, jobId, roleId) {
    const waveFile = findWaveFile(waveId);
    if (!waveFile)
        return;
    try {
        ...
        // Find entry by jobId
        const idx = data.roles.findIndex((r) => r.jobId === jobId);
        if (idx >= 0) {
            const entry = data.roles[idx];
...
/* ─── Helpers ─────────────────────────────── */
function watchJobCompletion(waveId, jobId, roleId) {
...
function collectChainEvents(jobChain, existingEvents) {
