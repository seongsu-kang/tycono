...
async function waitForOfficePage(page: Page) {
  ...
  await page.waitForSelector('button:has-text("CEO WAVE")', { timeout: 10_000 });
}
...
test.describe('TC-W01: WaveModal', () => {
  test('opens with propagation preview', async ({ page }) => {
    ...
    await page.click('button:has-text("CEO WAVE")');

    // Modal opens
    await expect(page.locator('text=Broadcast a directive')).toBeVisible();
...
test.describe('TC-W02: Wave Dispatch', () => {
  test('opens WaveCommandCenter after dispatch', async ({ page }) => {
    ...
    // Directive shown
    await expect(page.locator('text=Test directive for E2E').first()).toBeVisible();
...
test.describe('TC-W07: Side Panel During Wave', () => {
  test('can open role side panel while wave is minimized', async ({ page }) => {
    ...
    if (await pmCard.isVisible()) {
      ...
      // Side panel or role detail should be visible
      const visible = await page.locator('[data-side-panel]').or(page.locator('text=Persona')).first().isVisible({ timeout: 3_000 }).catch(() => false);
